...
// calculate total prices from 
// [{ price: 10, quantity: 2 }, { price: 5, quantity: 4 }] => [20, 20]
// (price * quantity)
...
const coordinates = function (object) {
...
// given an array of books, each with a `title` and `chapters`, where each 
// chapter has a `title` and a `pageCount`, return an array of books, each 
// containing the book's title and an array of chapter titles along with their
//  page counts
...
// given an array of events, where each event has a `name` and an array of 
// `attendees`, where each attendee has a `firstName` and `lastName`, 
// return an array of events where each event contains the event name and an 
// array of full names of attendees
